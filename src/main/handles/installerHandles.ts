    ...
    yauzl.open(PATH_zip, { lazyEntries: true }, (error, zipFile) => {
      ...
      zipFile.on('end', () => {
        mainWindow.webContents.send(
          'fixButtonProgress',
          'Extraction complete. Updating packages...'
        )
        // Delete the ZIP file
...
const backupPythonInstallation = async (
  ...
  mainWindow.webContents.send(
    'fixButtonProgress',
    `Finished downloading and installing required files!`
  )
  console.log('DONE')
  return
}

async function download_via_https(
  name: string,
  URL: string,
  file_path: string,
  mainWindow: Electron.BrowserWindow,
...
const reinstallPythonDependencies = async (
  artroomPath: string,
  gpuType: string,
  mainWindow?: Electron.BrowserWindow
) => {
  ...
  try {
    ...
    mainWindow?.webContents.send(
      'fixButtonProgress',
      `Failed to reinstall Python dependencies after ${MAX_INSTALL_RETRIES} retries.`
    )
  }
}
