...
const Body = () => {
  ...
  const handleGetProgress: SocketOnEvents['get_progress'] = useCallback(
    data => {
      setProgress((100 * data.current_step) / data.total_steps)
      setBatchProgress(
        (100 * (data.current_num * data.total_steps + data.current_step)) /
          (data.total_steps * data.total_num)
      )
    },
    []
  )

  const handleGetStatus: SocketOnEvents['get_status'] = useCallback(
    data => {
  ...
  const handleAddToQueue: SocketOnEvents['add_to_queue'] = useCallback(
    data => {
  ...
  const computeShardCost = () => {
    ...
    let estimated_price = Math.round(
      ((imageSettings.width * imageSettings.height) / (512 * 512)) *
        (imageSettings.steps / 50) *
        imageSettings.n_iter *
        10
    )
  ...
  const submitCloud = () => {
    ProtectedReqManager.make_post_request(
      `${ARTROOM_URL}/gpu/submit_job_to_queue`,
      imageSettings
    )
      .then((response: any) => {
  ...
  const getProfile = () => {
    ProtectedReqManager.make_get_request(`${ARTROOM_URL}/users/me`)
  ...
  return (
    <Box width="100%" alignContent="center">
      <VStack spacing={3}>
        ...
        >
          <ImageObj b64={mainImage?.b64} path={mainImage?.path} active />
        ...
        <Box maxHeight="120px" overflowY="auto" width="60%">
          <SimpleGrid minChildWidth="100px" spacing="10px">
            {latestImages.map((imageData, index) => (
              <Image
                h="5vh"
                key={index}
