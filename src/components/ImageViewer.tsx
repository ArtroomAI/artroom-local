...
function ImageViewer() {
  ...
  const [imageViewPath, setImageViewPath] = useRecoilState(
    atom.imageViewPathState
  )
  ...
  useEffect(() => {
    if (imageViewPath.length <= 1) {
      setImageViewPath(
        path.join(imageSettings.image_save_path, imageSettings.batch_name)
      )
    }
  }, [])
  ...
  return (
    <>
      ...
      <Box height="90%" ml="50px" p={5} rounded="md" width="75%">
        <Masonry
