...
export const stageCursorSelector = selector({
	...
	get: ({ get }) => {
		...
		const isTransformingBoundingBox = get(
			atoms.isTransformingBoundingBoxAtom,
		);
...
export const selectedImageLayerSelector = selector({
	key: 'selectedImageLayer.selector',
	get: ({ get }) => {
		const layer = get(atoms.layerAtom);
		const layerState = get(atoms.layerStateAtom);
		return layerState.images.find(elem => elem.id === layer);
	},
});
