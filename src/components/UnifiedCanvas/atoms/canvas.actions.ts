...
import { getNameForLayer, LAYER_REG_EXP } from '../../../utils';
...
// 			x: roundToMultiple(
// 				(newImage as Image).width / 2 -
// 					newBoundingBoxDimensions.width / 2,
// 				64,
// 			),
// 			y: roundToMultiple(
// 				(newImage as Image).height / 2 -
// 					newBoundingBoxDimensions.height / 2,
// 				64,
// 			),
// 		};
...
// 						LAYER_REG_EXP,
// 						atoms.initialLayerState.images,
// 					),
// 					isVisible: true,
// 					opacity: 1,
// 					picture: {
...
export const addLineAction = selector<number[]>({
	...
	set: ({ set, get }, payload) => {
		...
		const pastLayer = get(atoms.pastLayerAtom);
		...
		const newColor =
			layer !== 'mask' && tool === 'brush' ? { color: brushColor } : {};
	...
	set: ({ set, get }) => {
		...
		set(atoms.layerStateAtom, {
			...
			objects: [
				...
				{
					kind: 'fillRect',
					layer: layer, // maybe add check to exclude mask
					// ...boundingBoxCoordinates,
					x: boundingBoxCoordinates.x - imageLayerOffset.x,
					y: boundingBoxCoordinates.y - imageLayerOffset.y,
					...boundingBoxDimensions,
					color: brushColor,
				},
			],
...
export const resizeAndScaleCanvasAction = selector<void>({
	...
	set: ({ get, set }) => {
		const canvasContainerDimensions = get(
			atoms.canvasContainerDimensionsAtom,
		);
		const layerState = get(atoms.layerStateAtom);
		const boundingBoxScaleMethod = get(atoms.boundingBoxScaleMethodAtom);
...
export const redoAction = selector<void>({
	...
	set: ({ get, set }) => {
		...
		const layerState = get(atoms.layerStateAtom);
		const maxHistory = get(atoms.maxHistoryAtom);
		const futureLayer = get(atoms.futureLayerAtom);
		const layer = get(atoms.layerAtom);
		...
		if (futureLayer) {
			set(atoms.layerAtom, futureLayer);
			set(atoms.pastLayerAtom, layer);
			set(atoms.futureLayerAtom, '');
		}
...
export const undoAction = selector<void>({
	...
	set: ({ get, set }) => {
		...
		if (pastLayer) {
			set(atoms.layerAtom, pastLayer);
			set(atoms.futureLayerAtom, layer);
		}
