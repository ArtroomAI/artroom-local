...
type MessageType<K extends keyof SocketOnEvents> = Parameters<
  SocketOnEvents[K]
>[0]

function parseSocketMessage<T extends keyof SocketOnEvents>(
  type: T,
...
export const Console = () => {
  ...
  const handleMessage = useCallback(
    (
      type: keyof SocketOnEvents,
      message: MessageType<keyof SocketOnEvents>
    ) => {
  ...
  return (
    <>
      ...
      >
        ...
        <ModalContent>
          <ModalHeader />
          <ModalBody>{displayMessages()}</ModalBody>
        </ModalContent>
      </Modal>
    </>
  )
