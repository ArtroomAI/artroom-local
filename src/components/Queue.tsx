...
function Queue() {
  ...
  // handles
  const handleGetServerStatus: SocketOnEvents['get_server_status'] =
    useCallback(data => {
      setServerRunning(data.server_running)
    }, [])

  const handleGetQueue: SocketOnEvents['get_queue'] = useCallback(
    data => {
      setQueue(data.queue)
    },
    [setQueue]
  ...
  const handleStartQueue: SocketOnEvents['start_queue'] = useCallback(
    data => {
  ...
  const handlePauseQueue: SocketOnEvents['pause_queue'] = useCallback(
    data => {
      if (data.status === 'Success') {
  ...
  const handleStopQueue: SocketOnEvents['stop_queue'] = useCallback(
    data => {
      if (data.status === 'Success') {
        setServerRunning(false)
        toast({
          title: 'Stopped queue',
          description:
            'Current batch has been interrupted and queue has been stopped',
          status: 'success',
          position: 'top',
          duration: 4000,
          isClosable: false,
  ...
  return (
    <Box
      ...
      width="100%"
    >
      <Grid gap="24px" templateColumns="4fr 1fr">
        <Card overflowX="hidden" p="16px">
          ...
          <Table className="queue-table" color="#fff" variant="simple">
            <Thead>
              <Tr my=".8rem" ps="0px">
                <Th borderBottomColor="#56577A" color="gray.400">
                  #
                </Th>
