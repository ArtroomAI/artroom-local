  ...
  const handleGetImages = useCallback(
    (data: ImageState) => {
      if (
        ...
        latestImages[0].batch_id !== data.batch_id
      ) {
            ...
            for (let i = 0; i < job_list.length; i++) {
              for (let j = 0; j < job_list[i].images.length; j++) {
                if (job_list[i].images[j].status == 'PENDING') {
                  ...
                  let shard_refund =
                    job_list[i].image_settings.shard_cost /
                    job_list[i].image_settings.n_iter
                  toast({
                    title:
                      'Cloud Error Occurred, ' +
                      shard_refund +
                      ' Shards Refunded to account',
                    description:
                      'Failure on Image id: ' +
                      job_list[i].images[j].id +
                      ' Job id: ' +
                      job_list[i].id,
                    status: 'error',
                    position: 'top',
                  ...
                  window.api.saveFromDataURL(
                    JSON.stringify({ dataURL, imagePath })
                  )
                }
              }
            }
  ...
  return (
    <>
      ...
      >
        ...
        <GridItem area="header" justifySelf="center" pt="3">
          {loggedIn ? (
            <HStack align="center">
              <ProfileMenu setLoggedIn={setLoggedIn} />

              <VStack alignItems="center" spacing={0}>
                <Icon as={cloudMode ? IoMdCloud : IoMdCloudOutline} />
        ...
        <GridItem area="main" pl="2">
          <Flex>
            <Routes>
              ...
              <Route element={<EquilibriumAI />} path="/equilibriumai" />
