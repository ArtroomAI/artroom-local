...
const parseExifErrors = (errors: IValidation.IError[]) => {
  ...
  return errors.map((err) => {
    ...
    return (
      <Text>
        In {path} expected {type}, but got {value}
      </Text>
    )
  })
}
...
export const checkSettings = (
  exif: ExifValidation | null,
  models?: InternalModelsType
): CheckSettingsType => {
  if (exif === null) {
    return [
      DEFAULT_EXIF,
  ...
  if (exif.success) {
    if (models) {
      ...
      const hasLoras = exif.data.loras.map((lora) => models.loras.includes(lora.name))
