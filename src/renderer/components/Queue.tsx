...
import { Box, Flex, Text, Table, Thead, Th, Tr, Tbody, HStack, IconButton } from '@chakra-ui/react'
...
function Queue() {
  ...
  return (
    <Box className="queue" height="90%" ml="30px" p={4} rounded="md" width="100%">
      <Card overflowX="hidden" p="16px">
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <Table className="queue-table" color="#fff" variant="simple">
            <Thead>
              <Tr my=".8rem" ps="0px">
                <Th borderBottomColor="#56577A" color="gray.400"></Th>
            ...
            <Tbody>
              <SortableContext items={queue} strategy={verticalListSortingStrategy}>
                {queue.map((elem, index, arr) => (
                  <SortableItem key={elem.id} id={elem.id} index={index} row={elem} arr={arr} />
                ))}
              </SortableContext>
            </Tbody>
          </Table>
