...
const FolderComponent = ({
  ...
  return (
    <Flex
      ...
      alignItems="center"
    >
      <Icon as={GoFileDirectory} fontSize="9xl" justifyContent="center" display="block" />
...
const SelectDisks = () => {
  ...
  return (
    <Box w="10%">
      <Select
        onChange={(e) => setImageViewPath(e.target.value)}
        value={`${imageViewPath.split(path.sep)[0]}\\`}
      >
        {disks.map((disk) => {
          return (
    ...
    <Box m={4}>
      <Flex mb={4}>
        ...
        <Input
          ...
          value={imageViewPath}
          onChange={(e) => {
            setImageViewPath(e.target.value)
          }}
        />
      </Flex>
...
const Controls = ({
  ...
  return (
    <Box width="100%" pos="sticky" top="30px" p="2" bgColor="#080B16">
      <PathButtons />
...
function ImageViewer() {
  ...
  return (
    <>
      ...
      <Box height="90%" p={5} width="100%" pos="relative">
        ...
        >
          {imagePreviews
            ...
            .map((image, index) =>
              image.isFolder ? (
                <FolderComponent
                  directory_path={image.name}
                  directory_full_path={image.fullPath}
                  key={index}
                />
              ) : (
                <ImageComponent
                  ...
                  key={index}
                  onClick={setImage(image.fullPath)}
                />
              )
            )}
        </Masonry>
