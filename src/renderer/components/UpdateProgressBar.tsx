    ...
    if(info.percent && info.percent < 100) {
        return (
            <Box position="fixed" bottom="0" left="0" right="0" textAlign="center">
                ...
                <Text position="absolute" bottom="0" left="0" right="0" textAlign="center" textShadow={"-1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000"}>
                    Downloading update: {(info.transferred / 1000 / 1000).toFixed(2)}MB / {(info.total / 1000 / 1000).toFixed(2)}MB - {(info.bytesPerSecond / 1000 / 1000).toFixed(2)}MB/s
                </Text>
            </Box>
        );
    }
