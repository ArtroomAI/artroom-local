...
export const validateSeedWeights = (seedWeights: SeedWeights | string): boolean => {
    ...
    : Boolean(
        seedWeights.length &&
          !seedWeights.some((pair: SeedWeightPair) => {
            ...
            const isWeightValid =
              !isNaN(parseInt(weight.toString(), 10)) && weight >= 0 && weight <= 1
            return !(isSeedValid && isWeightValid)
          })
      )
}

export const stringToSeedWeightsArray = (string: string): Array<Array<number>> => {
  const stringPairs = string.split(',')
  const arrPairs = stringPairs.map((p) => p.split(':'))
  return arrPairs.map((p: Array<string>): Array<number> => [parseInt(p[0]), parseFloat(p[1])])
}
