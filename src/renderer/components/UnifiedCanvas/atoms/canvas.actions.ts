...
export const setInitialCanvasImageAction = selector<Image | null>({
  ...
  set: ({ get, set }, newImage) => {
    ...
    const newBoundingBoxDimensions = {
      width: roundDownToMultiple(_.clamp((newImage as Image).width, 64, 512), 64),
      height: roundDownToMultiple(_.clamp((newImage as Image).height, 64, 512), 64),
    }
    const pastLayerStates = get(atoms.pastLayerStatesAtom)
    const layerState = get(atoms.layerStateAtom)

    const newBoundingBoxCoordinates = {
      x: roundToMultiple((newImage as Image).width / 2 - newBoundingBoxDimensions.width / 2, 64),
      y: roundToMultiple((newImage as Image).height / 2 - newBoundingBoxDimensions.height / 2, 64),
    }

    if (boundingBoxScaleMethod === 'auto') {
      const scaledDimensions = getScaledBoundingBoxDimensions(newBoundingBoxDimensions)
    ...
    set(atoms.pastLayerStatesAtom, [...pastLayerStates, _.cloneDeep(layerState)])
...
export const addLineAction = selector<number[]>({
  ...
  set: ({ set, get }, payload) => {
    ...
    const shouldRestrictStrokesToBox = get(atoms.shouldRestrictStrokesToBoxAtom)
    const layerState = get(atoms.layerStateAtom)
    const pastLayerStates = get(atoms.pastLayerStatesAtom)
    const maxHistory = get(atoms.maxHistoryAtom)
...
export const setBoundingBoxCoordinatesAction = selector<Vector2d>({
  ...
  set: ({ set }, newValue) => {
    set(atoms.boundingBoxCoordinatesAtom, floorCoordinates(newValue as Vector2d))
  },
})
...
export const resetCanvasViewAction = selector<{
  ...
  set: ({ get, set }, newValue) => {
    ...
    if (width !== 0 && height !== 0) {
      ...
      const newScale = calculateScale(stageWidth, stageHeight, 512, 512, STAGE_PADDING_PERCENTAGE)

      const newCoordinates = calculateCoordinates(stageWidth, stageHeight, 0, 0, 512, 512, newScale)
...
export const resizeAndScaleCanvasAction = selector<void>({
  ...
  set: ({ get, set }) => {
    ...
    const { width: containerWidth, height: containerHeight } = canvasContainerDimensions
...
export const undoAction = selector<void>({
  ...
  set: ({ get, set }) => {
    ...
    set(atoms.futureLayerStatesAtom, [_.cloneDeep(layerState), ...futureLayerStates])

    if (futureLayerStates.length > maxHistory) {
      set(atoms.futureLayerStatesAtom, futureLayerStates.slice(0, futureLayerStates.length - 1))
    }
