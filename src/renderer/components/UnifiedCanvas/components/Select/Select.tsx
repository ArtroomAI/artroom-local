...
type ISelectProps = SelectProps & {
  ...
  validValues: Array<number | string> | Array<{ key: string; value: string | number }>
}
/**
  ...
  return (
    ...
    >
      ...
      <Tooltip label={tooltip} {...tooltipProps}>
        <ChakraSelect className="painter__select-picker" fontSize={fontSize} size={size} {...rest}>
          {validValues.map((opt) => {
            return typeof opt === 'string' || typeof opt === 'number' ? (
              ...
              <option key={opt.value} value={opt.value} className="painter__select-option">
                {opt.key}
              </option>
            )
          })}
