...
const addOutpaintingLayer = async (
  imageDataURL: string,
  maskDataURL: string,
  width?: number,
  height?: number
...
const PaintLatestImages = ({ focused }: { focused: boolean }) => {
  ...
  const addToCanvas = useCallback(
    (imageData: Partial<ImageState>) => {
      const scaledDimensions = getScaledBoundingBoxDimensions(boundingBoxDimensions)
...
const PaintQueueButtons = () => {
  ...
  const handleRunInpainting = useCallback(() => {
    ...
    const maskDataURL = generateMask(layerState.objects.filter(isCanvasMaskLine), boundingBox)
    ...
    addOutpaintingLayer(imageDataURL, maskDataURL, boundingBox.width, boundingBox.height)
      .then((combinedMask) => {
        toast({
    ...
    <>
      {cloudMode ? (
        ...
        >
          ...
          <Text pl={1}>{computeShardCost(imageSettings)}</Text>
        </Button>
      ) : (
        <HStack>
          <Button className="run-button" ml={2} onClick={handleRunInpainting} width="200px">
            Run
          </Button>
          <IconButton
