...
const Body = () => {
  ...
  const submitCloud = useCallback(() => {
    ProtectedReqManager.make_post_request(`${ARTROOM_URL}/gpu/submit_job_to_queue`, imageSettings)
      .then((response: any) => {
  ...
  const handleIntermediateImages = useCallback(
    (data: ImageState) => {
    ...
    <Box width="100%">
      <VStack align="center" spacing={4}>
        ...
        {cloudMode ? (
          ...
          <HStack>
            <Button className="run-button" ml={2} onClick={addToQueue} width="200px">
              Run
            </Button>
            <IconButton
