...
class StableDiffusion:
    ...
    def generate_image(
            self,
            key = ('ckpt', 'vae', 'tuple(loras)', 'controlnet_path'),
            prompts_data=None,
        ...
        for prompts in prompts_data:
            with autocast('cuda'):
                if self.models[key]['model'].v1 and self.models[key]['model'].control_model is None:
                    self.models[key]['model'].modelCS.to(self.device)
                if self.models[key]['model'].control_model is not None:
                    self.models[key]['model'].control_model.switch_devices(diffusion_loop=False)
                
                uc = None
                if cfg_scale != 1.0:
                    uc = self.models[key]['model'].modelCS.get_learned_conditioning(
                        negative_prompts_data)
                ...
                if len(weighted_prompt) > 1:
                    ...
                    for i in range(len(weighted_prompt)):
                        ...
                        c = torch.add(c, self.models[key]['model'].modelCS.get_learned_conditioning(
                            weighted_prompt[i][0]), alpha=weight)
                else:
                    c = self.models[key]['model'].modelCS.get_learned_conditioning(
                        prompts)
                ...
                if init_image is not None:
                    if self.models[key]['model'].v1 or self.models[key]['model'].control_model is not None:
                        self.models[key]['model'].modelFS.to(self.device)
                    if controlnet.lower() != "none":
                        control = init_image.clone()
                        c = {"c_concat": [control], "c_crossattn": [c]}
                        uc = {"c_concat": [control], "c_crossattn": [uc]}
                        self.models[key]['model'].control_model.control_scales = [1.0] * 13
                    else: 
                        init_image = repeat(
                            init_image, '1 ... -> b ...', b=batch_size)
                        init_latent_1stage = self.models[key]['model'].modelFS.encode_first_stage(init_image)
                        init_latent_1stage = init_latent_1stage.mode() if mode == "pix2pix" else init_latent_1stage
                        init_latent = self.models[key]['model'].modelFS.get_first_stage_encoding(init_latent_1stage).to(self.device)
                        x0 = self.models[key]['model'].model.stochastic_encode(
                            init_latent,
                            torch.tensor(
                    ...
                    if mask_image is not None:
                        ...
                        if self.models[key]['model'].v1:
                            if self.models[key]['model'].model.model1. \
                                    diffusion_model.input_blocks[0][0].weight.shape[1] == 9:
                ...
                if control is not None:
                    self.models[key]['model'].loading = False #Important that it goes here to not mess up preload
                    ddim_sampler = DDIMSampler(self.models[key]['model'].control_model)
                    ...
                    x0 = self.models[key]['model'].model.sample(
                            S=steps,
                            conditioning=c,
                            x0=x0,
                ...
                self.models[key]['model'].loading = False #Important that it goes here to not mess up preload
                x_samples_ddim = self.models[key]['model'].modelFS.decode_first_stage(
                    x0[0].unsqueeze(0))
