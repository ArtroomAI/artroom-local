async function getCkpts (data) {
    return await window.api.getCkpts(
        'getCkpts',
        data
    );
}
...
async function getImageFromPath (data) {
    return await window.api.getImageFromPath(
        'getImageFromPath',
        data
    );
}
...
async function copyToClipboard (B64) {
    return await window.api.copyToClipboard(
        'copyToClipboard',
        B64
    );
}
...
async function restartServer (isDebug) {
    return await window.api.restartServer(
        'restartServer',
        isDebug
    );
}
...
async function mergeModels (data) {
    return await window.api.mergeModels(
        'mergeModels',
        data
    );
}
