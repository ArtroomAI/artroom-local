...
let sd_data_json = JSON.parse(sd_data);
...
sd_data_json.ckpt = sd_data_json.ckpt.replace(
    '%InstallPath%',
    artroom_path
);
sd_data_json.ckpt = path.normalize(sd_data_json.ckpt);
sd_data_json.ckpt_dir = sd_data_json.ckpt_dir.replace(
    '%InstallPath%',
    artroom_path
);
sd_data_json.ckpt_dir = path.normalize(sd_data_json.ckpt_dir);
const debugModeInit = sd_data_json.debug_mode;
sd_data = JSON.stringify(
    sd_data_json,
    null,
    2
);
...
const baseCommand = `"${artroom_path}\\artroom\\miniconda3\\Scripts\\conda" run --no-capture-output -p "${artroom_path}/artroom/miniconda3/envs/artroom-ldm" python`;

const command = `${baseCommand} server.py`;

const mergeModelsCommand = `${baseCommand} model_merger.py`;
...
function createWindow () {
    ...
    ipcMain.handle(
        'copyToClipboard',
        (event, B64) => new Promise((resolve, reject) => {
    ...
    ipcMain.handle(
        'runPyTests',
        (event, argx = 0) => {
            const sd_path = `${artroom_path}\\artroom\\settings\\sd_settings.json`;
            const python_path = `${artroom_path}\\artroom\\miniconda3\\envs\\artroom-ldm\\python.exe`;
            if (!(fs.existsSync(artroom_install_log))) {
    ...
    ipcMain.handle(
        'restartServer',
        (event, isDebug) => new Promise((resolve, reject) => {
            kill(server.pid);
            spawn(
                'taskkill',
                ['/pid', server.pid, '/f', '/t']
